        ...
        stage('Secret Scanning'){
            steps{
                script{
                    ...
                    sh 'docker run --rm -i -v "$PWD:/repo" trufflesecurity/trufflehog:latest git file:///repo --no-update --entropy --regex --include-detectors="all" --json 1>&2 | tee trufflehog_output.json'
                        }
                    }
                }
